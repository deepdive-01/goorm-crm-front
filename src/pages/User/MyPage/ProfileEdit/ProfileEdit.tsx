...
export default function ProfileEdit(_props: ProfileEditProps = {}) {
  const { profile, isLoading, updateProfile } = useUserContext();
  // updateProfile 진행 중 여부 — 인디케이터 표시에 사용
  ...
  return (
    <VStack className="justify-start w-10/12 gap-10 pb-10">
      {/* 저장 중 인디케이터 */}
      {isUpdating && (
        <div className="flex items-center gap-2 text-gray-300 text-body4">
          <Spinner size={18} />
          저장 중...
        </div>
      )}
      ...
      <VStack className="gap-2">
        ...
        {isLoading ? (
          <Skeleton shape="square" className="w-40" />
        ) : (
          <Edit
            size="lg"
      ...
      <VStack className="gap-2">
        ...
        {isLoading ? (
          <Skeleton shape="square" className="w-56" />
        ) : (
          <Edit
            size="lg"
            inputClassName="w-full"
